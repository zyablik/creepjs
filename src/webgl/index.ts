...
export default async function getCanvasWebgl() {
	...
	try {
		...
		const getShaderPrecisionFormat = (gl, shaderType) => {
			...
			const LOW_INT = attempt(() => gl.getShaderPrecisionFormat(gl[shaderType], gl.LOW_INT))
			const MEDIUM_INT = attempt(() => gl.getShaderPrecisionFormat(gl[shaderType], gl.MEDIUM_INT))
		...
		const getWebGLData = (gl, contextType) => {
			...
			try {
				...
				console.log(`getWebGLData(${contextType}): drawingBufferWidth = ${drawingBufferHeight} drawingBufferHeight = ${drawingBufferHeight}`)
				let dataURI = ''
				...
				console.log(`getWebGLData(${contextType}): width = ${width} height = ${height}`)
